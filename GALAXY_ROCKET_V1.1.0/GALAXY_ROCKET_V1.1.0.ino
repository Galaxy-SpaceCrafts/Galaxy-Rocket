/*
GALAXY-ROCKET.ino
VERSION: V1.2.0 (Adaptive Gain & Ignition Delay Update)
---------------------------------------------------==============WARNING===============-------------------------------------------------
...
void loop() {
  ...
  // CONTROLE: Só move se detectou liftoff e passou o delay de segurança de 0.5s
...
float getAdaptiveScale() {
  ...
  // Conforme gráfico de CN-alpha: a eficiência aumenta até Mach 1.3 e depois cai.
  // Escalonamos os ganhos inversamente à pressão dinâmica para manter estabilidade.
  if (mach < 0.8) return 1.0;
  if (mach >= 0.8 && mach < 1.3) return 0.7; // Reduz ganho na zona transônica (vencer flutter)
  if (mach >= 1.3 && mach < 3.0) return 0.5; // Reduz ganho em supersônico (alta autoridade aerodinâmica)
  return 0.4;
}
...
void executeControl(float dt) {
  ...
  for(int i = 0; i < 4; i++) {
    // Aplicação do Ganho Adaptativo no Kp e Kd
    float p_term = (pidGains[i].kp * adaptiveScale) * error[i];
    ...
    float d_term = (pidGains[i].kd * adaptiveScale) * (error[i] - prev_err[i]) / dt;
  ...
  switch(flightPhase) {
    case 0: 
      if(telem.accelZ > 20.0) { 
        flightPhase = 1; 
        liftoff_millis = millis(); // Marca tempo real do liftoff
      } 
      break;
    case 1: 
...
// Funções de baixo nível mantidas (MPU, Checksum, LoRa, SD, etc)
...
void calculateGuidance(float dt) {
  ...
  if(!guide.target_acquired && guide.range_to_target < 10000 && telem.alt > 5000) guide.target_acquired = true;
  if (flightPhase >= 2 && guide.target_acquired) {
    ...
    if (los_rate > 180) los_rate -= 360; if (los_rate < -180) los_rate += 360;
