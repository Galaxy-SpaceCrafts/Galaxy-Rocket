...
void executeAirbrakes(float dt) {
  ...
  // Phase 2: Coasting (Inertial Ascent)
  if (flightPhase == 2 && K_vel > AB_VEL_THRESHOLD) {
    
    /* 
     * MAPPING LOGIC:
     * We start deploying at 150 m/s to avoid excessive drag at peak Mach,
     * reaching full deployment (AB_DEPLOYED) at 350 m/s.
     */
    targetPosition = constrain(map(K_vel, 150.0, 350.0, AB_RETRACTED, AB_DEPLOYED), AB_RETRACTED, AB_DEPLOYED);
    
    // Dynamic Slew Rate (Smooth movement to prevent aerodynamic flutter)
    // Faster retraction (120 deg/s) than deployment (80 deg/s) for stability
    if (currentABPosition < targetPosition) {
        currentABPosition += 80.0 * dt; 
    } else {
        currentABPosition -= 120.0 * dt;
    }

  } else {
    // Immediate retraction during Boost (Phase 1) or Descent (Phase 3+)
    currentABPosition = AB_RETRACTED;
  }

  // Symmetric Command to all 4 actuators
