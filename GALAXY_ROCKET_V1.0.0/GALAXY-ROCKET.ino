...
#define ABORT_ANGLE 45.0       // Ângulo de ataque máximo para aborto
...
// Telemetria Otimizada para Ground Station (Binário)
struct Telemetry {
  uint32_t timestamp;
...
unsigned long last_micros = 0, last_telem = 0, last_log = 0;
int flightPhase = 0;
...
float P_00 = 1, P_01 = 0, P_10 = 0, P_11 = 1; // Covariância
float Q_accel = 0.01; // Ruído de processo
float R_alt = 0.5;    // Ruído do sensor Baro
...
void setup() {
  ...
  Serial.println(F("=== GALAXY ROCKET - MISSION CONTROL ==="));
  Serial.print(F("TARGET: ")); Serial.print(TARGET_LAT, 6); Serial.print(", "); Serial.println(TARGET_LON, 6);
...
void loop() {
  ...
  // 1. SENSOR FUSION (Kalman + IMU)
  if (GROUND_TEST_MODE) simulateHITL(dt);
  else readSensorsKalman(dt);

  // 2. SEGURANÇA (Lógica de Aborto)
  checkAbortLogic();

  // 3. NAVEGAÇÃO E MISSÃO
  calculateGuidance(dt);
  updateFlightPhase();

  // 4. CONTROLE (Só executa se não houver aborto)
  if (!abortTriggered) {
    executeControl(dt);
    valveControl();
  } else {
    centerServos();
    digitalWrite(DROGUE_PIN, HIGH); // Eject imediato em caso de aborto
  }

  chuteLogic();

  // 5. TELEMETRIA (Interface Gráfica LoRa)
  if (millis() - last_telem > 50) {  // 20Hz p/ rádio
    sendTelemetry();
    last_telem = millis();
  }

  // 6. BLACKBOX BINÁRIA (100Hz)
...
// === FILTRO DE KALMAN (FUSÃO BARO + IMU) ===
void readSensorsKalman(float dt) {
  MPU6050_read();
  float measured_alt = bmp.readAltitude(1013.25);
  float accel_z = (ins.az * 9.81); // Aceleração vertical compensada
  ...
  // Atualização da Covariância pós-correção
  P_00 -= k0 * P_00;
  P_01 -= k0 * P_01;
  P_10 -= k1 * P_00;
  P_11 -= k1 * P_01;
  ...
  // Complementary Filter para Atitude
...
void checkAbortLogic() {
  ...
  if (flightPhase == 1 && total_aoa > ABORT_ANGLE) {
    ...
    flightPhase = 3; // Força fase de descida
    Serial.println(F("!!! CRITICAL ABORT: ANGLE OF ATTACK EXCEEDED !!!"));
    beep(10);
  }
}

// === BLACKBOX BINÁRIA (FAST LOG) ===
...
void logBinaryData() {
  ...
  dataFile = SD.open("LOG.DAT", FILE_WRITE);
  if(dataFile) {
    dataFile.write((const uint8_t*)&telem, sizeof(Telemetry));
    dataFile.close();
  }
}
...
// === NAVEGAÇÃO E CONTROLE (Originais preservados) ===
void calculateGuidance(float dt) {
  if(!gpsValid) return;
  float dLat = (TARGET_LAT - telem.lat) * PI / 180.0;
  float dLon = (TARGET_LON - telem.lon) * PI / 180.0;
  float a = sin(dLat/2)*sin(dLat/2) + cos(telem.lat*PI/180)*cos(TARGET_LAT*PI/180)*sin(dLon/2)*sin(dLon/2);
  guide.range_to_target = 6371000.0 * 2.0 * atan2(sqrt(a), sqrt(1-a));
  telem.dist_to_target = guide.range_to_target;

  float bearing_to_target = atan2(sin(dLon)*cos(TARGET_LAT*PI/180), cos(dLat) + sin(TARGET_LAT*PI/180)*sin(telem.lat*PI/180));
  float dlon_val = (telem.lon - TARGET_LON) * PI / 180 * cos(telem.lat * PI/180);
  guide.cross_track_error = asin(sin(dlon_val)*sin(bearing_to_target)) * 6371000;

  if(!guide.target_acquired && guide.range_to_target < 10000 && telem.alt > 5000) {
    guide.target_acquired = true;
  }

  if (flightPhase >= 2 && guide.target_acquired) {
    float target_bearing = bearing_to_target * 180/PI;
    float los_rate = (target_bearing - telem.heading);
    if (los_rate > 180) los_rate -= 360;
    if (los_rate < -180) los_rate += 360;
    guide.lateral_accel_cmd = 3.0 * fabs(telem.vel) * (los_rate * PI/180.0);
    guide.lateral_accel_cmd = constrain(guide.lateral_accel_cmd, -MAX_LAT_ACCEL, MAX_LAT_ACCEL);
...
void executeControl(float dt) {
  ...
  if (ref_yaw > 180) ref_yaw -= 360;
  if (ref_yaw < -180) ref_yaw += 360;

  float error[4] = { ref_roll - telem.roll, ref_pitch - telem.pitch, ref_yaw, guide.cross_track_error * 0.001f };

  for(int i = 0; i < 4; i++) {
    float p_term = pidGains[i].kp * error[i];
    ...
    float d_term = pidGains[i].kd * (error[i] - prev_err[i]) / dt;
    float pid_out = constrain(p_term + integrator[i] + d_term, -FIN_LIMIT, FIN_LIMIT);
    prev_err[i] = error[i];
    if(i == 0) applyActuatorPhysics(pid_out, pid_out, pid_out, dt);
  }
}

void applyActuatorPhysics(float r_cmd, float p_cmd, float y_cmd, float dt) {
  float t1 = constrain(90 + y_cmd * 0.8 + r_cmd * 0.2, 90-FIN_LIMIT, 90+FIN_LIMIT);
  float t2 = constrain(90 + p_cmd * 0.9 + r_cmd * 0.1, 90-FIN_LIMIT, 90+FIN_LIMIT);
  float t3 = constrain(90 - p_cmd * 0.7 + y_cmd * 0.3, 90-FIN_LIMIT, 90+FIN_LIMIT);
  float m = ACTUATOR_RATE * dt;
  last_fin_pos[0] += constrain(t1 - last_fin_pos[0], -m, m);
  last_fin_pos[1] += constrain(t2 - last_fin_pos[1], -m, m);
  last_fin_pos[2] += constrain(t3 - last_fin_pos[2], -m, m);
...
void updateFlightPhase() {
  ...
  switch(flightPhase) {
    case 0: if(telem.accelZ > 20.0) flightPhase = 1; break;
    case 1: mx_alt = max(mx_alt, telem.alt); 
            if(telem.alt > 45000) { digitalWrite(STAGE_SEP_PIN, HIGH); delay(50); digitalWrite(STAGE_SEP_PIN, LOW); flightPhase = 2; } break;
    case 2: if(telem.vel < -5.0) { digitalWrite(DROGUE_PIN, HIGH); flightPhase = 3; } break;
    case 3: if(telem.alt < 2000) { digitalWrite(MAIN_PIN, HIGH); flightPhase = 4; } 
            if(guide.target_acquired && guide.range_to_target < LANDING_RADIUS && telem.alt < 1000) landingMode = true; break;
    case 4: if(telem.alt < TARGET_SAFE_ALT + 20 && landingMode) { flightPhase = 5; centerServos(); }
            else if(telem.alt < 50) { flightPhase = 5; centerServos(); } break;
  }
...
void MPU6050_read() {
  ...
  ins.ax = (ax / 16384.0) - bias.ax; ins.ay = (ay / 16384.0) - bias.ay; ins.az = (az / 16384.0) - bias.az - 1.0;
  ins.gx = (gx / 131.0 * PI/180) - bias.gx; ins.gy = (gy / 131.0 * PI/180) - bias.gy; ins.gz = (gz / 131.0 * PI/180) - bias.gz;
}

void INS_calibrate() {
  for(int i = 0; i < 200; i++) { MPU6050_read(); delay(2); }
  bias.ax = ins.ax; bias.ay = ins.ay; bias.az = ins.az; bias.gx = ins.gx; bias.gy = ins.gy; bias.gz = ins.gz;
  saveEEPROM();
}
...
void checkGroundCommands() {
  if(LoRa.parsePacket()) {
    ...
    if(c[0] == 0xAA && c[1] == 0x05) abortTriggered = true; // Aborto remoto
  }
}
...
void beep(int n) { for(int i=0; i<n; i++) { digitalWrite(BUZZER, HIGH); delay(80); digitalWrite(BUZZER, LOW); delay(120); } }