/*
GALAXY-ROCKET.ino
VERSION: V1.4.0 (Airbrakes System Update)
---------------------------------------------------==============WARNING===============-------------------------------------------------
...
// Airbrake Servo Pins (Arduino Mega Digital Pins)
#define AB_PIN_1 22
#define AB_PIN_2 23
#define AB_PIN_3 24
#define AB_PIN_4 25
...
// Control Constants
const int AB_RETRACTED = 0;   // Degrees when closed
const int AB_DEPLOYED  = 85;  // Degrees when fully open (Max drag)
const float AB_VEL_THRESHOLD = 15.0; // Minimum velocity (m/s) to allow deployment
...
void setup() {
  ...
  MPU6050_init();
  INS_calibrate();
  airbrake1.attach(AB_PIN_1);
  airbrake2.attach(AB_PIN_2);
  airbrake3.attach(AB_PIN_3);
  airbrake4.attach(AB_PIN_4);

  // Set initial safe state (Closed)
  airbrake1.write(AB_RETRACTED);
  airbrake2.write(AB_RETRACTED);
  airbrake3.write(AB_RETRACTED);
  airbrake4.write(AB_RETRACTED);
...
/**
 * Manages the 4-fin airbrake system deployment based on flight phase and velocity.
 * Phase 2 is typically Coast (Inertial Ascent) after motor burnout.
 */
void executeAirbrakes(float dt) {
  float targetPosition = AB_RETRACTED;

  // LOGIC: Deploy ONLY during coast phase if velocity is safe
  if (flightPhase == 2 && K_vel > AB_VEL_THRESHOLD) {
    
    // Adjust 100.0 and 250.0 to your rocket's expected speed range
    targetPosition = constrain(map(K_vel, 100, 250, AB_RETRACTED, AB_DEPLOYED), AB_RETRACTED, AB_DEPLOYED);
    
    // Slew Rate Limiter: Prevents servo damage from sudden movements
    if (currentABPosition < targetPosition) currentABPosition += 60.0 * dt; 
    else currentABPosition -= 90.0 * dt;

  } else {
    // Force retraction during Boost, Apogee, and Descent
    currentABPosition = AB_RETRACTED;
  }

  // Update all 4 servos simultaneously for aerodynamic symmetry
  int outputPWM = (int)currentABPosition;
  airbrake1.write(outputPWM);
  airbrake2.write(outputPWM);
  airbrake3.write(outputPWM);
  airbrake4.write(outputPWM);
}
...
void loop() {
  ...
  if (!abortTriggered && flightPhase >= 1) {
    if (millis() - liftoff_millis > IGNITION_CONTROL_DELAY) {
      executeControl(dt);
      executeAirbrakes(dt); // <-- Dynamic Drag Control
    } else {
      centerServos();
    }
  }
  ...
  digitalWrite(AlarmLED, HIGH); // Liga o LED
  delay(100);                    // Espera 100 milissegundos (rápido)
  digitalWrite(AlarmLED, LOW);  // Desliga o LED
  delay(100);                    // Espera 100 milissegundos
  
  // Repete o piscar para efeito de alerta
  digitalWrite(AlarmLED, HIGH);
  delay(100);
  digitalWrite(AlarmLED, LOW);
  delay(500); // Pausa maior antes de recomeçar o alerta
